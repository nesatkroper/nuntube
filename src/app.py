...
class YouTubeDownloaderApp:
    def __init__(self, root):
        ...
        self.keep_mp4 = tk.BooleanVar(value=False)  # [NEW] Variable for Checkbox
    ...
    def create_ui(self):
        ...
        url_label.pack(pady=10, fill=tk.X, padx=10)
        ...
        path_label.pack(pady=10, fill=tk.X, padx=10)
        ...
        self.progress_bar.pack(pady=20, fill=tk.X, padx=10)

        log_label = ctk.CTkLabel(self.left_frame, text="Process Log:")
        log_label.pack(pady=5, fill=tk.X, padx=10)

        self.log_text = ctk.CTkTextbox(self.left_frame, height=200, width=500)
        self.log_text.pack(pady=10, fill=tk.BOTH, expand=True, padx=10)
        self.log_text.configure(state="disabled")

        # [NEW] Keep MP4 Checkbox
        self.keep_mp4_checkbox = ctk.CTkCheckBox(
            self.left_frame, text="Keep MP4 (Video file)", variable=self.keep_mp4
        )
        self.keep_mp4_checkbox.pack(pady=10, padx=10, anchor="w")

        self.download_button = ctk.CTkButton(
            self.left_frame, text="Start download MP3", command=self.start_download
        )
        self.download_button.pack(pady=20, fill=tk.X, padx=10)

        # --- Right Side (MP3 List) ---
        mp3_label = ctk.CTkLabel(self.right_frame, text="Downloaded MP3s")
        mp3_label.pack(pady=10, fill=tk.X, padx=10)

        reload_button = ctk.CTkButton(
            self.right_frame, text="Reload List", command=self.reload_mp3_list
        )
        reload_button.pack(pady=5, fill=tk.X, padx=10)

        browse_mp3_button = ctk.CTkButton(
            self.right_frame, text="Browse MP3 Folder", command=self.browse_mp3_folder
        )
        browse_mp3_button.pack(pady=5, fill=tk.X, padx=10)
        ...
        self.thumbnail_button.pack(pady=10, fill=tk.X, padx=10)
        ...
        self.mp3_scroll_frame.pack(pady=10, fill=tk.BOTH, expand=True, padx=10)

        # [NEW] Player UI at bottom (Height 200px)
    ...
    def create_player_ui(self):
        """Creates the fixed player area at the bottom of the right frame."""
        self.player_frame = ctk.CTkFrame(self.right_frame, height=150, fg_color="#2b2b2b")
        self.player_frame.pack(pady=10, fill=tk.X, padx=10, side=tk.BOTTOM)
        self.player_frame.pack_propagate(False) # Enforce height constraint
        ...
        self.now_playing_label.pack(pady=(15, 5), padx=10)
        ...
        # Stop Button
        self.btn_stop_player = ctk.CTkButton(
            controls_frame, 
            text="■", 
            width=40, 
            fg_color="#d63031",
        ...
        self.btn_stop_player.pack(side=tk.LEFT, padx=10)

        # Volume Slider
        vol_frame = ctk.CTkFrame(self.player_frame, fg_color="transparent")
        vol_frame.pack(pady=5)
        ctk.CTkLabel(vol_frame, text="Vol").pack(side=tk.LEFT, padx=5)
        self.vol_slider = ctk.CTkSlider(vol_frame, from_=0, to=1, command=self.set_volume, width=150)
    ...
    def play_mp3(self, file):
        ...
        try:
            ...
            self.now_playing_label.configure(text=f"Playing: {filename}")
    ...
    def stop_mp3(self, reset_ui=True):
        if self.playing_file or pygame.mixer.music.get_busy():
            pygame.mixer.music.stop()
            self.playing_file = None
            self.paused = False
        
        if reset_ui:
            self.now_playing_label.configure(text="Not Playing")
            self.btn_play_pause.configure(text="▶")
    ...
    def set_thumbnails(self):
        ...
        if not os.path.isdir(dir_to_use):
            messagebox.showerror("Error", "No valid directory set. Please Browse MP3 Folder first.")
            return
    ...
    def _process_thumbnails(self, mp3_files, image_path):
        ...
        for i, mp3_file in enumerate(mp3_files):
            try:
                ...
                pass # Skip faulty files quietly or log if needed

            progress_val = (i + 1) / total
            self.progress.set(progress_val)

        self.log_message("Thumbnails set!")
        self.root.after(0, lambda: messagebox.showinfo("Success", "Thumbnails updated!"))
    ...
    def download_channel(self, url):
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts_info) as ydl:
                ...
                channel_name = info.get("uploader") or info.get("title") or "Unknown_Channel"
            ...
            # [NEW] Check logic for keeping MP4
            should_keep_video = self.keep_mp4.get()
            ...
            ydl_opts = {
                ...
                # If True, keeps the downloaded video file. If False, deletes it (default behavior)
                "keepvideo": should_keep_video,
